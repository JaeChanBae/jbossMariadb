...
PD_GridUtils.fnTrClick = function(listId) {
  ...
  try {
    // 호출중에 창 닫으면 에러발생 대응
    activeBg = objTblBox.getAttribute("activeBg");
  } catch (e) {
 ...
 **/
PD_GridUtils.fnTrActiveOne = function(listId, rowIndex, bgColor) {
...
PD_GridUtils.fnFiexdGridResize = function(listId, listMargin) {
  ...
  // var fixed_width = parseInt(jQuery('#'+listId+'_FIXED_COL').attr('width'),
  // 10);
 ...
 * List Check All
 */
PD_GridUtils.fnListCheckAll = function(listId, bodyId, colId, ctrlFn, activeBg) {
  ...
  jQuery("#" + bodyId).find("input:checkbox[name=" + colId + "]").each(function(i) {
    ...
    if (activeBg && activeBg != 'N') {
      PD_GridUtils.fnTrActive(listId, idx, color);
    }
  });

...
PD_GridJson.makeScreenList = function(layoutPath, gridInfo, gridFields, xmlGalleryInfo, xmlGalleryField, listId, fixed) {
  ...
  var activeBg = gridInfo["activeBg"];
  ...
  // 2. 리스트 및 갤러리
  // 2.1 리스트
  appendHTML = this.makeScreenList_HTML(gridFields, listId, caption, fixed, wFixYn, width, height, listMargin, Number(pageLimit), cls, event, sort, ctrlFn, activeBg);
 ...
 * 리스트화면 HTML
 */
PD_GridJson.makeScreenList_HTML = function(gridFields, listId, caption, fixed, wFixYn, width, height, listMargin, pageLimit, cls, event, sort, ctrlFn, activeBg) {
...
//    if (th_width > 0 && th_sort == 'Y') {
  ...
  // #################### FIXED & LIST 합치기
  div_sb.append("<div id='" + listId + "_TBL_LIST_BOX' event='" + event + "' ctrlFn='" + ctrlFn + "' sort='" + sort + "' activeBg='" + activeBg + "' style='width:" + box_width_px + "; '>  \r\n");
...
PD_GridJson.fnFrameResize = function(arrParams) {
  ...
  case 'FRAME_HORIZONTAL_TWO_DIVISION':
    // 가로 2분할 프레임 리사이징
    var window_width = jQuery(".frame-wrapper").width();
    var frame_left_width = jQuery(".frame-left").width();
    var frame_center_width = jQuery(".frame-center").width();
    var frame_right_width = jQuery(".frame-right").width();
    var change_right_width = window_width - frame_left_width - frame_center_width;

    jQuery(".frame-right").width(change_right_width);

 ...
 * DESC : 페이지로딩 후 설정
 * VER  : 1.0
 * Copyright 2015 Hamee Korea Group All rights reserved
 * ============================================================================================
 *                변   경   사   항
 * ============================================================================================
...
PD_Init.initSelect = function() {
  /**------------------------------------------------------------------- Started **/
  /**
   * Next-Engine. Dynamic SelectBox Controll Library
   * @author 	park s.y
   * @since 	????/??/??
   */
  var selWidth = 0;
...
PD_Init.initJQueryFunction = function() {
  ...
  jQuery.fn.fnDragBarMoveEvent = function(arrParams) {
    ...
    case 'GRID_BOTTOM_BAR':
      ...
      this.bind('mousedown', function(e) {
        e.preventDefault();
        if (e.which != 1) {
          // e.which - 1:LEFT(MOUSE CLICK), 2:CENTER(MOUSE CENTER), 3:RIGHT(MOUSE RIGHT)
          return;
        }
    ...
    case 'FRAME_HORIZONTAL_TWO_DIVISION_BAR':
      ...
      var leftId = arrParams['left'];
      var rightId = arrParams['right'];
      ...
      this.bind('mousedown', function(e) {
        ...
        var beforeLeftWidth = parseFloat(jQuery("#" + leftId).width(), 10);
        var beforeRightWidth = parseFloat(jQuery("#" + rightId).width(), 10);
        ...
        jQuery(document).bind('mousemove', function(e) {
          ...
          //				        console.log("###### beforeLeftWidth : " + beforeLeftWidth + " beforeRightWidth : " + beforeRightWidth + " changeLeftWidth : " + changeLeftWidth + " changeRightWidth : " + changeRightWidth);

          jQuery("#" + leftId).width(changeLeftWidth);
          jQuery("#" + rightId).width(changeRightWidth);
  ...
  jQuery.fn.fnGridResize = function(arrParams) {
    ...
    this.bind('mousedown', function(e) {
      /*
        var relX = e.pageX - jQuery(el).offset().left;
        var relY = e.pageY - jQuery(el).offset().top;
        var maxX = jQuery('body').width() - jQuery(el).width() - 10;
        var maxY = jQuery('body').height() - jQuery(el).height() - 10;
       */
      e.preventDefault();
      if (e.which != 1) {
      ...
      jQuery(document).bind('mousemove', function(e) {
        /*
          ...
          jQuery(el).css('left', diffX + 'px').css('top', diffY + 'px');
         */
        var afterPosY = parseFloat(e.pageY);
  ...
  jQuery.fn.fnDatatablesResize = function(arrParams) {
    ...
    this.bind('mousedown', function(e) {
      ...
      jQuery(document).bind('mousemove', function(e) {
        ...
        //jQuery("#" + listId + "_wrapper .dataTables_scrollBody").css("z-index","2");
        ...
        if (jQuery("#" + listId + "_wrapper .dataTables_scrollBody").hasScrollBar().load) {
          $('#' + listId).dataTable().fnAdjustColumnSizing(false);
        }

        //가로 스크롤 한번더 처리
        //var vScroll = jQuery("#" + listId + "_wrapper .dataTables_scrollBody").hasScrollBar().vertical;
